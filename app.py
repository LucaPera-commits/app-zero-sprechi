...
db_url = "postgresql://postgres.incwdmenairgbqcvbhk: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@db.incwdmenairgbqcvbhk.supabase.co:5432/postgres"
...
def get_db_connection():
    try:
        ...
        st.error(f"🔴 Errore di connessione al Database Supabase: {e}")
        st.info("💡 Verifica di aver inserito la password corretta nell'URL del database!")
        return None
...
def carica_da_db():
    ...
    try:
        ...
        st.error(f"Errore caricamento dati dal Database: {e}. Hai eseguito la query SQL su Supabase?")
        return pd.DataFrame()
    ...
    if key_clean.startswith("gsk_"):
        ...
        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "system",
                    "content": "Sei uno Chef Nutrizionista esperto Anti-Spreco.",
                },
...
def cerca_alimento_api(query):
    ...
    headers = {
        "User-Agent": "ZeroSprecoApp - StreamlitClient/10.1 (contact@zerospreco.app)"
    }
...
def sezione_ricerca():
    ...
    modalita = st.radio(
        ...
        [
            "🔎 Cerca Confezionati (Database)",
            "🥬 Cibo Fresco Manuale (Senza Scadenza)",
        ],
        horizontal=True,
    )
    ...
    if modalita == "🔎 Cerca Confezionati (Database)":
        ...
        if risultati:
            opzioni = {
                f"{p['nome']} ({p['marca']}) - {p['kcal_100g']} kcal": p
                for p in risultati
            }
            scelta = st.selectbox(
                "Seleziona il cibo esatto:", list(opzioni.keys())
            )
            ...
            with col_cat:
                cat_scelta = st.selectbox(
                    "Categoria:", CATEGORIE_LISTA, index=0
                )
            with col_open:
                giorni_dopo_apertura = st.number_input(
                    "Giorni max d'uso da APERTO:",
                    min_value=1,
                    max_value=30,
                    value=3,
                )
            ...
            scadenza_input = st.date_input(
                "Data Scadenza Confezione Chiusa:",
                value=datetime.date.today() + datetime.timedelta(days=7),
            )
            quantita = st.number_input(
                "Quantità:", min_value=1, value=1, key="q_db"
            )
        ...
        with col_n:
            nome_fresco = st.text_input(
                "Nome Cibo Fresco:",
                placeholder="es. Insalata, Mele, Fettine di Pollo",
            )
        with col_m:
        ...
        with col_g:
            giorni_stima = st.number_input(
                "Giorni stimati di durata:",
                min_value=1,
                max_value=60,
                value=4,
            )
        with col_q:
            qta_fresco = st.number_input(
                "Quantità / Porzioni:", min_value=1, value=1, key="q_fr"
            )

        if st.button("💾 Salva Cibo Fresco"):
            if nome_fresco.strip():
                data_stimata = datetime.date.today() + datetime.timedelta(
                    days=giorni_stima
                )
                salva_in_db(
...
def sezione_dispensa():
    ...
    if not df.empty:
        ...
        if not urgenti.empty:
            st.error(
                f"🚨 **ATTENZIONE:** Hai {len(urgenti)} alimento/i da consumare subito!"
            )
...
def mostra_tabella_cibi(df_subset, prefix=""):
    for _, row in df_subset.iterrows():
        col_info, col_mancanti, col_aperto, col_qta, col_btn1, col_btn2 = (
            st.columns([3, 2, 1.5, 1, 1, 1])
        )
        ...
        with col_aperto:
            if row.get("aperto") == 0:
                if st.button(
                    "🔓 Apri",
                    key=f"{prefix}_open_{item_id}",
                ):
                    segna_come_aperto_db(
                        item_id,
                        giorni_max_aperto=row.get("giorni_da_aperto", 3),
                    )
                    st.rerun()
            else:
...
def sezione_ai_chef():
    ...
    if df.empty:
        st.info(
            "Aggiungi prima qualche alimento in dispensa per creare le ricette!"
        )
        return
    ...
    with col_a:
        num_adulti = st.number_input(
            "👨‍💼 Numero Adulti:", min_value=1, max_value=10, value=2
        )
    with col_b:
        num_bambini = st.number_input(
            "👶 Numero Bambini:", min_value=0, max_value=10, value=2
        )

    col_ka, col_kb = st.columns(2)
    with col_ka:
        kcal_adulto = st.number_input(
            "🔥 Kcal / Adulto:", min_value=200, max_value=1500, value=600
        )
    with col_kb:
        kcal_bambino = st.number_input(
            "👶🔥 Kcal / Bambino:",
            min_value=100,
            max_value=1200,
            value=380,
    ...
    st.caption(
        f"📊 **Target Totale Calcolato:** ~{kcal_totali_pasto} kcal ({num_adulti}x {kcal_adulto} kcal + {num_bambini}x {kcal_bambino} kcal)"
    )
    ...
    if st.button("🪄 Genera con l'IA Chef", type="primary"):
        if not ai_key:
            st.error(
                "⚠️ Inserisci la tua API Key nella barra laterale a sinistra!"
            )
            return

        alimenti_list = [
            f"- {row['nome']} (Categoria: {row['categoria']}, Scade tra {row['Giorni Rimasti']} giorni, Aperto: {'Sì' if row['aperto']==1 else 'No'}, Qtà: {row['quantita']})"
        ...
        try:
            with st.spinner("🧑‍🍳 L'IA sta creando la ricetta perfetta..."):
                st.session_state["ultima_ricetta"] = genera_ricetta_ai(
                    prompt, ai_key
                )
        except Exception as e:
